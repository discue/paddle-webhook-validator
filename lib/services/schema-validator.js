...
const validator = (data) => {
    ...
    return Object.keys(data).every((key) => {
        ...
        if (!value) {
            ...
            if (!matches) {
                console.error(`${data[key]} of key ${key} does not match pattern ${value.pattern}.`)
            }
            return matches
            ...
            if (!isUri) {
                console.error(`${data[key]} of key ${key} is not a valid URI pattern.`)
            }
            return isUri
            ...
            if (!shorterThanMaxLength) {
                console.error(`${data[key]} of key ${key} is invalid because length is greater than ${value.maxLength}.`)
            }
            ...
            if (!isOneOfEnumerations) {
                console.error(`${data[key]} of key ${key} is not one allowed enum values ${value.enum}.`)
            }
            ...
            if (!isDefaultValue) {
                console.error(`${data[key]} of key ${key} is not equal to default value ${value.default}.`)
            }
            ...
            if (!isLengthOk) {
                console.error(`${data[key]} of key ${key} is longer than 200 chars.`)
                return false
            }
            return isLengthOk
        }
