...
const SCHEMA_ALERT_MAP = SCHEMAS.reduce((context, next) => {
    context[next.properties.alert_name.default] = Object.entries(next.properties)
    return context
}, {})

const SCHEMA_ALERT_KEY_ONLY_MAP = SCHEMAS.reduce((context, next) => {
    context[next.properties.alert_name.default] = Object.keys(next.properties)
    return context
}, {})

const ALLOWED_HTTP_HOSTS = ['yoga', 'sandbox-my.paddle.com']
const ALLOWED_HTTPS_HOSTS = ['paddle.com']
...
module.exports = (data) => {
    ...
    const schema = SCHEMA_ALERT_MAP[name]
    const keys = SCHEMA_ALERT_KEY_ONLY_MAP[name]

    Object.keys(data).forEach((key) => {
        const keyInSchema = keys.includes(key)
        if (!keyInSchema) {
            delete data[key]
        }
    })

    return schema.every(([key, value]) => {
        if (value.pattern) {
...
function _validateUri(data, key) {
    return IF_NOT_EMPTY(data[key], (target) => {
        ...
        if (url.protocol === 'http:') {
            allowedHosts = ALLOWED_HTTP_HOSTS

        } else if (url.protocol === 'https:') {
            allowedHosts = ALLOWED_HTTPS_HOSTS
